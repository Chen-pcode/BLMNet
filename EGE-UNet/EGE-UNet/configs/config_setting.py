...
class setting_config:
    """
    训练配置集中放在这个类中，train.py 会直接读取这些类属性。
    修改实验设置通常优先改这里。
    """

    network = 'egeunet'
    # 模型结构配置：
    # num_classes=1 表示二分类分割；c_list 是 6 个编码/解码阶段的通道数。
    # bridge=True 使用 group aggregation bridge；gt_ds=True 使用 ground-truth guided deep supervision。
    model_config = {
        'num_classes': 1, 
        'input_channels': 3, 
    ...
    # 当前默认使用 ISIC2017。若切换数据集，需要保证 data_path 下目录结构一致。
    datasets = 'isic17' 
    if datasets == 'isic18':
    ...
    # 默认损失 = BCE + Dice，并叠加多个深监督输出的辅助损失。
    # 如果关闭 model_config['gt_ds']，这里也应改为 BceDiceLoss(wb=1, wd=1)。
    criterion = GT_BceDiceLoss(wb=1, wd=1)

    pretrained_path = './pre_trained/'
    num_classes = 1
    # 所有输入图像和 mask 会 resize 到该尺寸。
    input_size_h = 256
    input_size_w = 256
    input_channels = 3
    distributed = False
    ...
    # 每次运行都会新建一个带时间戳的结果目录，避免覆盖历史实验。
    work_dir = 'results/' + network + '_' + datasets + '_' + datetime.now().strftime('%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/'

    # print_interval: 训练日志打印间隔；val_interval: 完整验证指标计算间隔。
    # threshold: 将概率图二值化为分割 mask 的阈值。
    print_interval = 20
    val_interval = 30
    save_interval = 100
    threshold = 0.5

    # 训练增强：归一化 -> 转 tensor -> 随机翻转/旋转 -> resize。
    # 这里 image 和 mask 成对传入，空间变换会同时作用于二者。
    ...
    # 验证/测试阶段只做确定性的归一化、转 tensor、resize。
    ...
    # 下方保留多种优化器的参数分支，实际只会使用 opt 指定的那一支。
    if opt == 'Adadelta':
